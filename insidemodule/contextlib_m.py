...
# contextlib - 上下文管理 - 测试中可用

# 在Python中，读写文件这样的资源要特别注意，必须在使用完毕后正确关闭它们。正确关闭文件资源的
# 一个方法是使用try...finally：
'''
try:
...
# @contextmanager这个decorator接受一个generator，用yield语句把with ... as var把变量输出出去，然后，
# with语句就可以正常地工作了：
with create_query2('Bob') as q:
    q.query()
